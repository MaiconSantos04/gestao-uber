...
# --- GRÁFICOS (VOLTA DO DESIGN ORIGINAL) ---
...
# --- ÁREA FIPE COM ABAS (CORRIGIDO) ---
st.sidebar.info(f"🚗 Atual: **{config.get('fipe_nome_carro', '---')}**\n\n💰 Valor Base: **R$ {config.get('valor_carro', 0):,.2f}**")

tab_auto, tab_man = st.sidebar.tabs(["🔍 FIPE Auto", "✍️ Manual"])
            ...
            if nm_modelo:
                ...
                if anos:
                    ...
                    if st.button("💾 Buscar e Salvar"):
                        ...
                        else: st.error("Erro ao buscar valor.")
                else: st.info("Carregando anos...")
        else: st.warning("Erro ao carregar modelos. Use a aba Manual.")

with tab_man:
    n_man = st.text_input("Nome Carro", config.get('fipe_nome_carro',''), key="nm_man")
...
v_fixo = st.sidebar.number_input("IPVA+Seguro Anual", value=float(config.get('custo_fixo_anual', 6000)))
dias_mes = st.sidebar.number_input("Dias Trab/Mês", value=int(config.get('dias_trabalho_mes', 4)))
km_med = st.sidebar.number_input("Média KM/Dia", value=float(config.get('media_km_dia', 150)))
pr_gas = st.sidebar.number_input("Preço Gasolina", value=float(config.get('preco_gasolina', 5.80)))
cons = st.sidebar.number_input("Consumo (km/l)", value=float(config.get('consumo_carro', 10)))
c_manut = st.sidebar.number_input("Manut/km", value=float(config.get('custo_manut_km', 0.25)))
c_deprec = st.sidebar.number_input("Deprec/km", value=float(config.get('custo_deprec_km', 0.40)))
...
st.sidebar.divider()
st.sidebar.metric("⛔ STOP LOSS (Min/KM)", f"R$ {stop_loss:.2f}")
...
if menu == "📝 Lançamento Diário":
    ...
    ganhos = c1.number_input("Ganhos (R$)", 0.0, step=10.0)
    bonus = c2.number_input("Bônus (R$)", 0.0, step=10.0)
    km = c3.number_input("KM Rodado", 0.0, step=5.0)
    gas = c4.number_input("Combustível (R$)", 0.0, step=10.0)
    ...
    k1.error(f"🚨 GUARDAR: R$ {dia_guardar:.2f}")
    if dia_lucro > 0: k2.success(f"💵 LUCRO: R$ {dia_lucro:.2f}")
    else: k2.error(f"💸 PREJUÍZO: R$ {dia_lucro:.2f}")
    
    media_hj = (ganhos+bonus)/km if km>0 else 0
    k3.metric("Média Hoje", f"R$ {media_hj:.2f}/km", delta=f"{media_hj - stop_loss:.2f} vs Min")
    ...
    if c_save.button("💾 Salvar Lançamento", type="primary"):
        if ganhos or bonus:
            df_new = pd.DataFrame([{'Data': date.today(), 'Ganhos': ganhos, 'Bonus': bonus, 'Km_Rodado': km, 'Gastos_Combustivel': gas, 'Obs': obs}])
            salvar_dados(df_new); st.success("Salvo!"); st.cache_data.clear()
        else: st.warning("Preencha valores.")
    if c_undo.button("↩️ Desfazer Último"):
        desfazer(); st.toast("Desfeito!"); st.cache_data.clear()
    ...
    if not df.empty:
        ...
        del_id = st.number_input("ID para excluir", 0, step=1)
        if st.button("❌ Excluir Linha"):
            excluir_linha(del_id); st.success("Excluído!"); st.cache_data.clear(); st.rerun()
    ...
    if not df.empty:
        if menu == "📅 Relatório Semanal":
            ...
            df['Chave'] = df['Data'].apply(lambda x: f"{x.strftime('%b').capitalize()}/{x.year}") # Simplificado para evitar erro de map
            tit = "Mensal"
        else:
            df['Chave'] = df['Data'].apply(lambda x: str(x.year))
            tit = "Anual"

        # Agrupamento e Cálculos Completos
        ...
        gdf = res.rename(columns={'Ganhos':'Corridas', 'Bonus':'Bônus', 'Lucro_Liquido':'Lucro Real', 'IPVA_Guardado':'IPVA', 'Manut_Guardada':'Manut', 'Deprec_Guardada':'Deprec'})
        
        t1, t2, t3 = st.tabs(["💰 Faturamento", "💵 Lucro", "📉 Custos"])
        
        with t1:
            fig = px.bar(gdf, x='Chave', y=['Corridas', 'Bônus'], title="Receita Bruta", barmode='stack', color_discrete_map={'Corridas':'#00CC96', 'Bônus':'#636EFA'})
            st.plotly_chart(estilo_grafico(fig, "R$"), use_container_width=True)
        
        with t2:
            fig = px.bar(gdf, x='Chave', y='Lucro Real', title="Lucro Líquido Real", color_discrete_sequence=['#28a745'])
            st.plotly_chart(estilo_grafico(fig, "R$"), use_container_width=True)
            
        with t3:
            fig = px.bar(gdf, x='Chave', y=['IPVA', 'Manut', 'Deprec'], title="Detalhamento do Dinheiro a Guardar", barmode='group')
            st.plotly_chart(estilo_grafico(fig, "R$"), use_container_width=True)
    else:
